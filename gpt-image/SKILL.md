---
name: gpt-image
description: "Use this skill only when the user explicitly asks for the official gpt-image skill, official OpenAI Platform API image generation, the packaged `gpt-image` CLI, or the bundled GPT Image 2 reference gallery/craft library. Do not use for generic Chinese requests like 生图, 画图, 生成图片, 做图, or ordinary image generation in this user's setup; those should use `openai-next-image` instead."
