...
当用户提到飞书、Feishu、Lark、多维表格、bitable、飞书文档、飞书群、飞书日历、飞书消息、飞书任务等，并且需要通过 CLI 执行飞书操作时，优先使用本机固定入口：

```powershell
C:\Users\86180\.codex\bin\lark-cli.cmd
```

如果当前 shell 找不到 `lark-cli`，不要重新安装；先用上面的绝对路径运行。例如：

```powershell
C:\Users\86180\.codex\bin\lark-cli.cmd --version
C:\Users\86180\.codex\bin\lark-cli.cmd auth status
C:\Users\86180\.codex\bin\lark-cli.cmd doctor
```

该入口转发到 `C:\Users\86180\Documents\飞书\.pnpm-home` 下已安装的 `@larksuite/cli`，并自动把 Codex 自带 Node 加入该命令的 PATH。

配置/登录缺失时：

```powershell
C:\Users\86180\.codex\bin\lark-cli.cmd config init --new --lang zh
C:\Users\86180\.codex\bin\lark-cli.cmd auth login --recommend
C:\Users\86180\.codex\bin\lark-cli.cmd auth status
```
